...
# Dataset paths
DATASET_ROOT = r"C:\Users\zhuqh\Desktop\sam3\datasets\心脏超声数据集\18.心脏超声(EchoCP)\archive\EchoCP_dataset"
OUTPUT_DIR = r"C:\Users\zhuqh\Desktop\sam3\datasets\心脏超声数据集\EchoCP_coco"

# COCO categories
# Based on the pixel values [0, 1, 2, 3, 4] found in the labels
# Assuming standard labels similar to CardiacUDC:
# 1: Left Ventricle (LV)
...
def process_file_list(file_list, subset_name):
    ...
    for img_path, label_path in tqdm(file_list, desc=f"Processing {subset_name}"):
        try:
            ...
            # Check if 3D or 4D. Usually (H, W, T) or (H, W, 1, T) or just (H, W) if single frame
            if len(img_data.shape) == 3:
                num_frames = img_data.shape[-1]
            elif len(img_data.shape) == 4:
                num_frames = img_data.shape[-1] # Assuming last dim is frames
                # If 4D, maybe (H, W, Z, T). Let's check logic.
                # Usually cardiac echo is 2D+T -> (H, W, T)
            else:
            ...
            for i in range(num_frames):
                ...
                # Same rotation as previous script
    ...
    coco_output = {
        "info": {
            "description": f"EchoCP Dataset - {subset_name}",
            "year": 2024,
...
def get_patient_id(filename):
    # Filename format: 001_r_image.nii.gz or 001_v_image.nii.gz
    # Extract the first part (number)
...
def main():
    ...
    # Search for files
    img_files = glob(os.path.join(DATASET_ROOT, "*_image.nii.gz"))
    for img_path in img_files:
        # Construct expected label path
        # Assuming label is same but with _label instead of _image
